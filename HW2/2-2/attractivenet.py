...
class AttractiveNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bigramcnn = nn.Sequential(
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=215, kernel_size=config['kernel_size']-1, padding=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=215, out_channels=100, kernel_size=config['kernel_size']-1, padding=1),
            nn.ReLU(),
            nn.Dropout(config['dropout'])
        )

        self.trigramcnn = nn.Sequential(
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=215, kernel_size=config['kernel_size'], padding=1),
            nn.ReLU(),
            nn.Conv1d(in_channels=215, out_channels=100, kernel_size=config['kernel_size'], padding=1),
            nn.ReLU(),
            nn.Dropout(config['dropout'])
        )
