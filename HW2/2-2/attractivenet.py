...
class AttractiveNet(nn.Module):
    def __init__(self, config):
        ...
        self.bigramcnn = nn.Sequential(
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=config['embedding_dim'], kernel_size=config['kernel_size']-1, padding=1),
            # nn.ReLU(),
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=config['embedding_dim'], kernel_size=config['kernel_size']-1, padding=1),
            nn.ReLU(),
        ...
        self.trigramcnn = nn.Sequential(
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=config['embedding_dim'], kernel_size=config['kernel_size'], padding=1),
            # nn.ReLU(),
            nn.Conv1d(in_channels=config['embedding_dim'], out_channels=config['embedding_dim'], kernel_size=config['kernel_size'], padding=1),
            nn.ReLU(),
            nn.Dropout(config['dropout'])
        ...
        self.encoder_bigram_first = nn.LSTM(input_size=config['embedding_dim'], hidden_size=config['hidden_dim'], num_layers=config['num_layers'], dropout=config['dropout'], bidirectional=True, batch_first=True)
        self.encoder_trigram_first = nn.LSTM(input_size=config['embedding_dim'], hidden_size=config['hidden_dim'], num_layers=config['num_layers'], dropout=config['dropout'], bidirectional=True, batch_first=True)
