...
class AttractiveNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.bigramcnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=220, out_channels=100, kernel_size=config['kernel_size']-1, padding=1),
            nn.ReLU(),
        ...
        self.trigramcnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(in_channels=220, out_channels=100, kernel_size=config['kernel_size'], padding=1),
            nn.ReLU(),
            nn.Dropout(config['dropout'])
        )
